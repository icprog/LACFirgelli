...
int main(void)
{
	...
	static const int PRODUCT_ID = 0xfc5f;
	...
	if (result >= 0){
		...
		if (devh != NULL){
			...
			libusb_detach_kernel_driver(devh, INTERFACE_NUMBER);
			{
				...
				if (result >= 0){	device_ready = 1;}
				else{			fprintf(stderr, "libusb_claim_interface error %d\n", result);
				}
			}
		}
		else{ 	fprintf(stderr, "Unable to find the device.\n");}
	}
