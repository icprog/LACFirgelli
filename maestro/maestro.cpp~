...
static int test_control_transfer(void){
   	int r,i;
   	unsigned char answer[PACKET_CTRL_LEN-1];
   	unsigned char question[PACKET_CTRL_LEN-1];
   	for (i=0;i<PACKET_CTRL_LEN; i++) question[i]=0x20+i;
	r = libusb_control_transfer(devh,CTRL_OUT,HID_SET_POSITION,(HID_REPORT_TYPE_FEATURE<<8)|0x00, 0,question, PACKET_CTRL_LEN,TIMEOUT);
	if (r < 0) {
